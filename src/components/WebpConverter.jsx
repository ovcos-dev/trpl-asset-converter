    ...
    const convert = async () => {
        ...
        for (const item of selectedFiles) {
            try {
                ...
                const result = await ipcRenderer.invoke('convert-webp', {
                    buffer: arrayBuffer,
                    quality,
                    resize: resizeOption,
                })
                ...
                setResults(prev => [...prev, {
                    name: item.file.name.replace('.png', `${suffix}.webp`),
    ...
    return (
        <div className="tool-panel">
            <div
                ...
                onClick={() => document.getElementById('webp-input').click()}
            >
            ...
            <button onClick={convert} disabled={!selectedCount || loading}>
                {loading ? '변환 중...' : `WebP 변환 시작 ${selectedCount > 0 ? `(${selectedCount}개)` : ''}`}
            </button>
